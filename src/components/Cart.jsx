  ...
  return (
    ...
    "translate-x-0":"translate-x-full"} transition-all duration-500 z-50`}>
     <div className='flex justify-between items-center my-3'>
        ...
        <IoMdClose onClick={()=> setActiveCart(!activeCart)} className='border-2 border-gray-600 font-bold 
        p-1 text-xl rounded-md hover:text-red-500 hover:border-red-700
        cursor-pointer'/>
     </div>
     {
     ...
     <div className='absolute bottom-0'>
      ...
      rounded-lg w-[90vw] lg:w-[18vw] mb-5'>Checkout</button>
      
    </div>
    </div>
    < BsCartFill 
    onClick={()=>setActiveCart(!activeCart)}
    className={`rounded-full hover:bg-green-500 cursor-pointer bg-white shadow-md text-5xl p-3 fixed bottom-6 right-4 ${totalQty>0 && "animate-bounce delay-500 transition-all"}`}/>
    </>
  )
}
